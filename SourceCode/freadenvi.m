...
%
%     Reads an image of ENVI standard type(only fit bsq format)
...
% p  1 by 3 vector that contains (1) the number of cols, (2) the number.
...
while 1
    ...
    switch first
        ...
        case elements(5)
            [~,s]=strtok(second);
            interleave=s(2:end);
            if strcmp(interleave,'bsp')
                error('interleave type error(the function only fit on bsp fomart)');
            end
    end
end
fclose(rfid);
...
disp([('Opening '),(num2str(p(1))),(' cols x '),(num2str(p(2))),(' lines x '),(num2str(p(3))),(' bands'), ...
    (' of type '), (t), (' image')]);
disp([('interleave '),(':'),interleave]);
fid=fopen(strcat(fname,'.img'),'r');
if fid == -1
